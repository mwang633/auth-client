  ...
  def apply(ctx: RequestContext): Future[Authentication[Session]] = {
    ...
    match {
      case Some(token) =>
        ...
        authClient
          .checkPermission(token, ctx.request.method.toString(), ctx.request.uri.path.toString())
          .map(authResponse => {
            authResponse.session match {
              ...
              case None =>
                log.info(s"Permission denied for in request for uri ${ctx.request.uri}.")
                Left(AuthorizationFailedRejection)
            }
          })

      case None =>
        log.info(s"No session cookie found in request for request ${ctx.request}")
  ...
  private def getSessionToken(ctx: RequestContext): Option[String] = {
    ...
    if (ctx.request.method != HttpMethods.GET) {
      ...
      (cookieToken, headerToken) match {
        ...
        case _ =>
          log.info(s"Found mismatching header and cookie tokens for request ${ctx.request}")
          None
      }
    }
  }
